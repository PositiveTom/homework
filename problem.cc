...
bool Problem::Solve(int iterations) {
    ...
    std::fstream f("/home/zjj/MyCode/VIO-Homework/CurveFitting_LM/data/data2.txt", ios::out);
    if(!f.is_open())
    {
        cout << "error" << endl;
        return -1;
...
//  具体上面一部分的具体逻辑是什么，之后再做细看，现在先抓重点
    ...
    if (rho > 0 && isfinite(tempChi))   // last step was good, 误差在下降
    {
        ...
        currentChi_ = tempChi;
        currentLambda_ = std::max(currentLambda_/Ldown, 1e-7);
        return true;
        ...
        currentLambda_ = std::min(currentLambda_*Lup, 1e7);
        return false;
